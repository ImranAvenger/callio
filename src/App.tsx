    ...
    return (
      <main className="flex h-dvh min-h-0 flex-col overflow-hidden bg-[#0e1110] text-[#f5f1eb]">
        ...
        <section className="relative mx-[3vw] my-5 min-h-0 flex-1 flex items-center justify-center overflow-hidden rounded-2xl border border-[#263029] bg-[#101412] max-sm:mx-3 max-sm:my-3">
          <video className="max-h-full max-w-full object-contain" style={{ aspectRatio: remoteAspectRatio }} ref={attachRemoteVideo} onLoadedMetadata={(event) => setRemoteAspectRatio(event.currentTarget.videoWidth / event.currentTarget.videoHeight || 16 / 9)} onResize={(event) => setRemoteAspectRatio(event.currentTarget.videoWidth / event.currentTarget.videoHeight || 16 / 9)} autoPlay playsInline />
