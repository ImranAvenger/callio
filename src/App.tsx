...
function App() {
  ...
  return <main className="min-h-screen bg-[#111313] text-[#f5f1eb]"><Toast message={call.error} /><header className="flex h-22 items-center justify-between border-b border-white/7 px-[6vw] max-sm:h-17.5"><Brand /><span className="text-[13px] text-[#858d87] max-sm:hidden">Simple calls. Better together.</span></header><section className="mx-auto mt-[11vh] w-[min(560px,88vw)] max-sm:mt-[10vh]"><div className="font-mono text-[11px] uppercase tracking-[.08em] text-[#afbdac]">● Private video calls, made easy</div><h1 className="my-6 text-[clamp(58px,8vw,100px)] font-semibold leading-[.94] tracking-[-.075em]">Be there,<br /><em className="font-display font-medium tracking-[-.06em] text-[#c9e181]">together.</em></h1><p className="mb-10 text-base leading-[1.65] text-[#9da39d]">A quiet place for the conversations that matter.<br />Create a room and invite someone in.</p><label className="mb-2 block font-mono text-[11px] uppercase tracking-widest text-[#778078]" htmlFor="name">Your name</label><input id="name" className="w-full rounded-md border border-[#303934] bg-[#1a1e1c] px-4 py-3.5 outline-none focus:border-[#a7c776]" value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Alex" maxLength={40} /><button className="mt-4 flex w-full items-center gap-4 rounded-md bg-[#c9e181] px-4 py-3.5 text-left text-[#152018]" onClick={createRoom}><span className="text-[27px]">＋</span><span className="flex-1"><strong className="block">Create a room</strong><small>Start a new private call</small></span><b>→</b></button><div className="my-5 flex items-center gap-3 font-mono text-[11px] text-[#667069]"><span className="h-px flex-1 bg-[#29302c]" />or<span className="h-px flex-1 bg-[#29302c]" /></div><div className="flex gap-2 max-sm:flex-col"><input className="flex-1 rounded-md border border-[#303934] bg-[#1a1e1c] px-4 py-3.5 uppercase tracking-[.12em] outline-none" value={joinKey} onChange={(event) => setJoinKey(event.target.value.toUpperCase())} onKeyDown={(event) => event.key === "Enter" && joinRoom()} placeholder="Enter room code" maxLength={6} /><button className="rounded-md border border-[#3e4940] px-4 py-3.5 max-sm:py-3.5" onClick={joinRoom}>Join room →</button></div><p className="mt-8 text-center font-mono text-[10px] text-[#657068]">No account needed <span className="px-2">•</span> Your call is peer-to-peer</p></section></main>;
}
